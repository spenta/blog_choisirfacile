	...
	if (!is_single()) {
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			switch(intval($pagenavi_options['style'])) {
				case 1:
					if(!empty($pages_text)) {
						echo '<span class="pages">Page ' . $paged .' sur ' . $max_page . '</span>';
					}
					if ($start_page >= 2 && $pages_to_show < $max_page) {
						$first_page_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $pagenavi_options['first_text']);
						echo '<a href="'.clean_url(get_pagenum_link()).'" class="first" title="'.$first_page_text.'">&laquo; Premier</a>';
					...
					if ($end_page < $max_page) {
						...
						echo '<a href="'.clean_url(get_pagenum_link($max_page)).'" class="last" title="'.$last_page_text.'">Dernier  &raquo;</a>';
					}
					break;
				case 2;
...
function pagenavi_init() {
	...
	$pagenavi_options['pages_text'] = __('Pages %CURRENT_PAGE% sur %TOTAL_PAGES%','wp-pagenavi');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = __('&laquo; Premier','wp-pagenavi');
	$pagenavi_options['last_text'] = __('Dernier &raquo;','wp-pagenavi');
